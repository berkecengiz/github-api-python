...
class GitHubAPI:
    ...
    def create_comment(
        ...
        data = {"body": body}
        url = BASE_URL + f"{owner}/{repo}/issues/{issue_number}/comments"
        response = requests.post(url, json=data, headers=self.headers)
        ...
        if response.status_code == 201:
            ...
            message = response.json()["message"]
            raise ValueError(f"{message} (status code: {response.status_code})")
    def search_issues(self, owner: str, repo: str, query: str) -> list:
        # Encode the query string for use in the URL
        query = quote(query)
        url = f"{BASE_URL}{owner}/{repo}/issues?q={query}"
        response = requests.get(url, headers=self.headers)
