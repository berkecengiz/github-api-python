...
class GitHubAPI:
    ...
    def update_issue(self, owner: str, repo: str, issue_number: int, title: str = "", body: str = "",) -> dict:
        data = {}
        if title:
            data["title"] = title
        if body:
...
def main():
    ...
    for issue in issues:
        ...
        last_update = datetime.datetime.strptime(issue["updated_at"], "%Y-%m-%dT%H:%M:%SZ")
